...
public class RatingJoinReducerNoFilter extends Reducer<Text, JoinGenericWritable, Text, DoubleWritable> {
	public void reduce(Text key, Iterable<JoinGenericWritable> values, Context context)
		...
		for (JoinGenericWritable v : values) {
			...
			if(!pRecord.Genres.toString().toUpperCase().contains("COMEDY"))
				continue;
